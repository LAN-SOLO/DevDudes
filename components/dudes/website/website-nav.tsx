...
import { STEP_CATEGORIES, STEP_LABELS } from '@/lib/website-pipeline/constants'

export function WebsiteNav() {
  const { currentStep, setCurrentStep, visibleSteps } = useWebsiteWizard()
        ...
        {filteredCategories.map((category) => {
          ...
          return (
            <div key={category.id} className="flex flex-col gap-1">
                    ...
                    <div key={stepNum} className="flex items-center">
                      <button
                        ...
                        className={cn(
                          ...
                          isStepActive && 'bg-rose-500 text-white',
                          isStepComplete && 'bg-rose-500/80 text-white',
                      ...
                      {idx < category.steps.length - 1 && (
                        <div
                          className={cn(
                            'h-0.5 w-2',
                            isStepComplete ? 'bg-rose-500/60' : 'bg-muted'
                          )}
                        />
                      )}
                    </div>
              ...
              {isActive && (
                <span className="text-xs text-rose-500 font-medium px-1 truncate max-w-[120px]">
